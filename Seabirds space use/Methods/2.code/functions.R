...
plot_vul <- function(grid_obj, mean, cv, world_obj = europe, xlim_obj = xlim, ylim_obj = ylim){ 
  ...
  p1 <- ggplot() +
    geom_sf(data = grid_obj, aes(fill = mean), color = NA, lwd = 0) +
    geom_sf(data = world_obj) +
    geom_sf(data = zone_prio, fill = NA, lwd = 1, col = "black") +
    geom_sf(data = zone_concu, fill = NA, lwd = 1, col = "black") +
    geom_sf(data = zone_10, fill = NA, lwd = 1, col = "black") +
    geom_sf(data = zone_50, fill = NA, lwd = 1, col = "black") +
    xlim(xlim_obj) +
    ylim(ylim_obj) + 
    scale_fill_gradientn(colours= c("#1b263b","#009392", "#fbc99d", "#ffa86b","#ff9286","#d17b88") ,
                         # breaks = c(min(mean), max(mean)),
                         # labels = c("Low", "High"),
                         guide = guide_colourbar(
                           title = "Vulnerability",
                           title.theme = element_text(
  ...
  p2 <- ggplot() +
    geom_sf(data = grid_obj, aes(fill = cv), color = NA, lwd = 0) +
    geom_sf(data = world_obj) +
    xlim(xlim_obj) +
    ylim(ylim_obj) + 
    scale_fill_gradientn(colours= c("#f8edeb","#e8ddb5","#f7af9d","#e8a598","#b56576","#6d597a","#355070") ,
  ...
  #cowplot::plot_grid(p1, p2, labels = c("A. Répartition spatiale", "B. Incertitude"), ncol = 2)
